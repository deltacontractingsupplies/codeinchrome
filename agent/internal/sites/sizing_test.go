...
func TestWorkersFitThePlansMemory(t *testing.T) {
	for limit, want := range map[string]int{
		...
		if got := WorkersFor(limit); got != want {
...
func TestTheDatabaseAlwaysAllowsMoreConnectionsThanTheWorkers(t *testing.T) {
	for _, limit := range []string{"256m", "512m", "1024m", "2048m"} {
		if ConnectionsFor(limit) <= WorkersFor(limit) {
			t.Errorf("%s: %d connections for %d workers - a burst would fail on max_user_connections",
				limit, ConnectionsFor(limit), WorkersFor(limit))
		}
	}
}
...
func TestTheImageUsesTheSameFormula(t *testing.T) {
	b, err := os.ReadFile("../../../infra/images/laravel-8.3/Dockerfile")
	if err != nil {
		t.Fatal(err)
	}
	s := string(b)
	for _, want := range []string{
		fmt.Sprintf("workers=$(( (mb - %d) / %d ))", sizingBaseMB, sizingPerWorkerMB),
		fmt.Sprintf(`[ "$workers" -lt %d ] && workers=%d`, minWorkers, minWorkers),
		fmt.Sprintf(`[ "$workers" -gt %d ] && workers=%d`, maxWorkers, maxWorkers),
		"then mb=1024",
	} {
		if !strings.Contains(s, want) {
			t.Errorf("the Dockerfile's cic-start does not contain %q - it must match sizing.go", want)
		}
	}
}
