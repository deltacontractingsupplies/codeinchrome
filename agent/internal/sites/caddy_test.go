...
func TestEveryAppResponseIsCheckedForDownloadsAndOffsiteRedirects(t *testing.T) {
	...
	for _, s := range []Site{
		...
		for _, want := range []string{"@cic_download expression", "@cic_redirect_bad expression", "handle_response {",
			...
			"@cic_worker header Service-Worker script", "-Refresh", "header_up X-Real-IP {client_ip}", "header_up -X-Cic-Probe", `header @cic_static ?Cache-Control "public, no-cache"`, "immutable"} {
