...
- [x] T2. A headless browser for the agent: visit pages as a visitor, click
      ...
      PART 2026-09-26 (agent 0.49.0): cic.screens(path) - the page at phone, tablet and desktop size, side by side over the editor, so one screenshot checks all three; rendered on another host than the site's in the link scanner's locked-down browser (a real screenshot proved on h4: 390x844 in 2.4 s). Behind the app's login too (2026-09-26, agent 0.50.0): cic.screens(path, { as }) - one sign-in link per size, each opened by the renderer. Real device sizes and a measured sideways-scroll check (2026-09-26, agent 0.51.0): Chromium's window cannot be narrower than 500 px, so the 'phone' had been a 500 px page cropped to 390 - found while building the check. The renderer now drives Chromium's DevTools device emulation (shots.py): a phone is a real 390 px layout, one browser run for all three sizes (7 s), and each size says how wide its content is - cic.screens(...).overflow lists every size that scrolls sideways (proved on a host: 608 px of content on a 390 px phone).
...
- [x] **A7 [high]** Customer sites are not on a separate Public Suffix List domain (owner-deferred item, re-raised) DECIDED 2026-09-26 (owner): keep customer sites on codeinchrome.com for now (the dashboard's cookies are __Host- and cannot be set from a site); revisit later.
...
- [x] **A30 [medium]** Ban evasion: only the canonical email links a banned person to a new account, and Google/Apple sign-up skips the Gmail-only domain rule - PART DONE 2026-09-25: network signals as in A19. Google and Apple sign-up without the Gmail-only rule is the owner's decision (any real Google or Apple identity). DECIDED 2026-09-26 (owner): any real Google or Apple identity may sign up (Google and Apple verify the person); ban evasion is covered by the network and device signals (A19). Already how the code behaves.
...
- [ ] Rotate what lived on a laptop (defence in depth - none is in the
      ...
      2026-09-26 (owner approved): the Cloudflare token ROLLED in the dashboard, installed in the operator .env and the control host, verified there (tokens/verify, WAF read). Lemon Squeezy: a new API key (expires 26 Sep 2028 - rotate before then) replaces the old one; the webhook secret rotates without a missed payment (the app accepts the previous secret too while LEMONSQUEEZY_WEBHOOK_SECRET_PREVIOUS is set; remove it once the store has the new one).
...
- [x] **Platform mail from its own IP** (the control host): a mail provider
      (Postmark, Resend) instead keeps the host's address out of every mail
      header and SPF record.
      DECIDED 2026-09-26 (owner): keep sending from the control host (SPF and DKIM set up).
