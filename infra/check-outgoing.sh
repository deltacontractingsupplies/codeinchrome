...
# The same list as infra/publish-repo.sh. CIC_OWNER_NOTIFY_EMAIL is the owner's
# delta address, which every commit carries as its author by the owner's choice.
PUBLIC_KEYS='^(APP_NAME|APP_ENV|APP_URL|APP_DEBUG|CLOUDFLARE_ZONE_NAME|MAIL_(MAILER|HOST|PORT|SCHEME|FROM_ADDRESS|FROM_NAME)|LOG_.*|DB_CONNECTION|SESSION_.*|CACHE_STORE|QUEUE_CONNECTION|APPLE_CLIENT_ID|CIC_OWNER_NOTIFY_EMAIL|SHOWCASE_(URL|ADMIN_URL|ADMIN_EMAIL|FLOWERS_URL|FLOWERS_ADMIN_URL|FLOWERS_ADMIN_EMAIL|RECORDING))$'
...
for envfile in "$root/.env" "$root/control/.env"; do
  ...
  while IFS= read -r line; do
    ...
    # Unchanged from the committed example (Laravel's defaults: REDIS_CLIENT,
    # AWS_DEFAULT_REGION, ...): public by definition, never a secret.
    grep -qxF -- "$line" "$envfile.example" 2>/dev/null && continue
